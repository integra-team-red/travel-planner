...
public class CityController {
    ...
    @Operation(
            summary = "Get a list of all cities from the repository",
            responses = {
                @ApiResponse(
                        responseCode = "200",
                        description = "All cities returned successfully",
                        content =
                                @Content(
    ...
    @Operation(
            summary = "Add a city to the repository",
            responses = {
                @ApiResponse(
                        responseCode = "200",
                        description = "City added successfully; returns the added city",
                        content =
                                @Content(
                                        mediaType = "application/json",
    ...
    @Operation(
            summary = "Delete a city from the repository",
            responses = {
                @ApiResponse(responseCode = "200", description = "City deleted successfully", content = @Content)
            })
    ...
    @Operation(
            summary = "Update a city's data with the provided city",
            responses = {
                @ApiResponse(
                        responseCode = "200",
                        description = "City updated successfully; returns the updated city",
                        content =
                                @Content(
                                        mediaType = "application/json",
                                        schema = @Schema(implementation = CityDTO.class))),
    ...
    @Operation(
            summary = "Download a JSON file of all cities from the repository to the user's computer",
            responses = {
                @ApiResponse(
                        responseCode = "200",
                        description = "Cities returned for download successfully",
                        content =
                                @Content(
                                        mediaType = "application/json",
                                        array = @ArraySchema(schema = @Schema(implementation = CityDTO.class))),
                        headers = {
                            @Header(
                                    ...
                                    schema = @Schema(example = "attachment; filename=\"ApprovedCities.json\""))
                        }),
            })
    ...
    @Operation(
            summary = "Upload an array of cities formatted in JSON to add to the repository",
            responses = {
                @ApiResponse(
                        responseCode = "200",
